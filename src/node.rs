...
impl Node {
    ...
    pub async fn run(mut self) {
        ...
        match self.sync_state {
            SyncState::IBD(IBDState::HeaderSync(_)) => {
                ...
                match self.tcp_conn_channel.1.recv().await.unwrap().payload() {
                    NetworkMessage::Version(version) => {
                        let block_hash = BlockHash::from_byte_array(self.headers_chain.tip().1);
                        let msg_to_send = self.build_get_headers_msg(block_hash).unwrap();
                        tcp_sender_clone.send(msg_to_send.clone()).unwrap();

                        let target_height = version.start_height;
                        info!("got best height {} from version message", target_height);
        ...
        match msg.payload() {
            NetworkMessage::Block(block) => {
                ...
                if valid {
                    ...
                    // check if we have processed 500 blocks, if so, ask for next 500. NOTE: not ideal
                    // doing it this way
            ...
            NetworkMessage::Headers(headers) => {
                ...
                self.headers_chain.process_headers(block_headers)?;
                ...
                match &self.sync_state {
                    SyncState::IBD(IBDState::HeaderSync(sync_progress)) => {
                        ...
                        if new_sync_height >= sync_progress.target_height {
                            ...
                            let block_hash = BlockHash::from_byte_array(self.headers_chain.tip().1);
                            let raw_headers_msg = self.build_get_headers_msg(block_hash)?;
    ...
    fn build_get_headers_msg(
        &self,
        locator_hash: BlockHash,
    ) -> Result<RawNetworkMessage, NodeError> {
        let get_headers_msg =
            GetHeadersMessage::new(vec![locator_hash], BlockHash::from_byte_array([0; 32]));
        let raw_headers_msg =
