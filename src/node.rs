use std::{collections::HashMap, fs, process, sync::Arc};
...
pub struct Node {
    ...
    best_height_peer: Option<Receiver<Height>>,
    network: Network,
...
impl Node {
    ...
    pub fn new(
        ...
        let sync_state = match read_txn.open_table(HEADER_TIP_TABLE) {
            Ok(table) => {
                // if we are here, need to get the height and them compare to tip from chain
                // manager
        ...
        // full blocks will be processed by the chainstate manager
        ...
        let (best_height_sender, best_height_receiver) = unbounded();
        let best_height_peer = if let SyncState::IBD(IBDState::HeaderSync) = &sync_state {
            Some(best_height_receiver)
        } else {
            None
        };

        // if in header sync, we are starting from scratch
        let headers_chain = if let SyncState::IBD(IBDState::HeaderSync) = &sync_state {
            // TODO: start it with genesis block
            let mut headers_chain = HashMap::new();
            ...
            let genesis_block_header = BlockHeader {
                ...
                previous_block: genesis_block.header.prev_blockhash,
                merkle_root: *genesis_block.header.merkle_root.as_ref(),
                next_block: None,
                height: 0,
            };

            headers_chain.insert(genesis_block.block_hash(), genesis_block_header);
            headers_chain
        } else {
    ...
    pub async fn run(mut self) {
        ...
        let best_height = match self.sync_state {
            SyncState::IBD(IBDState::HeaderSync) => {
                let height_clone = self.best_height_peer.clone();
                height_clone.unwrap().recv().unwrap()
            }
            _ => 0,
        };
        ...
        loop {
            crossbeam_channel::select! {
                ...
                recv(self.block_header_receiver) -> msg => {
                    ...
                    for block_header in block_headers {
                        if self.valid_header(block_header) {
                            ...
                            let new_block_header = BlockHeader{
                                ...
                                previous_block: block_header.prev_blockhash,
                                merkle_root: *block_header.merkle_root.to_raw_hash().as_ref(),
                                next_block: None,
                    ...
                    // do not send intermediate progress for header sync
                    if header_sync_height == best_height {
                        ...
                        // TODO: if we are finished with the header sync, save it to db
    ...
    // save current state of headers chain to db
    // along with tip
