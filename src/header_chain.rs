...
impl HeadersChain {
    ...
    pub fn process_headers(&mut self, headers: Vec<Header>) -> Result<(), HeaderChainError> {
    ...
    pub fn save_headers_chain(&self) -> Result<(), HeaderChainError> {
        ...
        let write_tip_txn = self.db.begin_write()?;
        {
            ...
            info!("saved header tip {:?}", self.headers_chain_tip);
        }
    ...
    pub fn get_block_inventory(
        ...
        for _ in 0..=500 {
            let block_hash = current_block.next_block.as_ref().unwrap();
            if let NextBlock::Chain(hash) = block_hash {
